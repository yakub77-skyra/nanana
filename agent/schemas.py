...
class Scene(BaseModel):
    type: Literal["title_card", "map_intro", "news_frame", "article_card", "location_highlight",
                   ...
                   "quote_card", "stat_overlay", "clip", "map", "article", "quote", "breaking"]
    narration: str = ""
...
class StorySchema(BaseModel):
    scenes: list[Scene] = Field(description="Scenes. Start with title_card or map_intro, then mix of news_frame/article_card/location_highlight/quote_card/stat_overlay. End with breaking_card.")
    caption: str = ""
    hashtags: list[str] = []

class RoundupScene(BaseModel):
    headline: str = Field(description="5-6 word English ALL CAPS headline")
    narration: str = Field(description="1 sentence Hinglish narration")
    image_query: str = Field(description="Generic visual search query")
    location: str = "INDIA"
    state: str = Field(description="Indian state name (Rajasthan/Karnataka etc). Empty if national.", default="")

class RoundupSchema(BaseModel):
    intro_narration: str = "आइए जानते हैं भारत में पिछले 24 घंटों में क्या हुआ"
    scenes: List[RoundupScene] = Field(description="Exactly 8 headlines")
    caption: str
    hashtags: List[str]
