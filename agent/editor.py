...
FEED_IMAGES = []  # Populated by nodes.py before rendering
...
def _ken_burns_mp4(img_path, out_path, dur):
    """Turn a static photo into a slow-zoom video (ensures frames always move)."""
...
def render_scene(scene, i, vo=None, fmt="deep_dive"):
    ...
    if scene.type == "title_card":
        ...
        # Ken Burns injection: turn static photo into moving video
        video_b64, video_mime = "", "video/mp4"
        img_path = os.path.join(settings.output_dir, f"photo_{i}.jpg")
        if photo_b64 and os.path.exists(img_path):
            kb_path = os.path.join(settings.output_dir, f"kb_{i}.mp4")
            try:
                _ken_burns_mp4(img_path, kb_path, dur)
                if os.path.exists(kb_path):
                    video_b64 = fx._b64_or_empty(kb_path)
        ...
        html = fx.article_card_html(
            ...
            scene.date_str or "",
            bg_b64, dur, source_color=scene.source_color or "#111"
        )
        webm = fx.record_html(html, dur, f"ac{i}")
        _seg_mux(webm, vo, out, dur)

    elif scene.type == "keyword_text":
        html = fx.keyword_text_html(scene.keyword or scene.headline or "NEWS", _get_footage_b64(scene, i, dur) or "", dur)
        webm = fx.record_html(html, dur, f"kw{i}")
        _seg_mux(webm, vo, out, dur)

    elif scene.type == "stat_callout":
        html = fx.stat_callout_html(
            scene.stat_text or "0", scene.stat_label or "",
            _get_footage_b64(scene, i, dur) or "", dur,
            theme=scene.theme or "purple",
            extra_lines=scene.extra_lines or [],
            photo_b64=_get_photo_b64(scene, i)
        )
        webm = fx.record_html(html, dur, f"sc{i}")
        _seg_mux(webm, vo, out, dur)

    elif scene.type == "table_card":
        html = fx.table_card_html(scene.table_title or "DATA", scene.table_rows or [], dur)
        webm = fx.record_html(html, dur, f"tb{i}")
        _seg_mux(webm, vo, out, dur)

    elif scene.type == "location_highlight":
        photo_b64 = _get_photo_b64(scene, i)
        html = fx.location_highlight_html(
            scene.country or "India", scene.pin or scene.location or "LOCATION",
            photo_b64, scene.overlay_text or scene.headline or "", dur, theme=scene.theme or "red"
        )
        ...
        html = fx.disaster_dramatic_html(
            ...
            scene.sub_text or scene.breaking_sub or "", _get_footage_b64(scene, i, dur), dur
        )
        ...
        html = fx.footage_highlight_html(
            _get_footage_b64(scene, i, dur),
            scene.circle_x or 540, scene.circle_y or 960, scene.circle_r or 200,
            scene.label_text or "", dur
        )
        ...
        html = fx.breaking_card_html(
            ...
            scene.breaking_sub or "", img_b64, dur, source=scene.masthead or ""
        )
        ...
        html = fx.stat_overlay_html(scene.stat_text or "0", scene.stat_label or "",
                                    _get_footage_b64(scene, i, dur) or "", dur, theme=scene.theme or "purple")
