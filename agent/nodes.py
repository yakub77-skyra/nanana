...
def _real_url(link):
    """Google News redirect → real publisher URL."""
...
def _cut(s, n):
    ...
    Smart cut:
    - word-safe
    - no dangling 'THE', 'OF', 'HOSPITAL'S'
    - prevents ugly hooks like 'MUMBAI HOSPITAL'S'
    """
    s = s or ""
    ...
    while words and (
        words[-1].upper().strip(".") in bad
        or words[-1].upper().endswith("'S")
        or words[-1].endswith(",")
        or words[-1].endswith(":")
        or words[-1].endswith(";")
    ):
...
def select_story(state):
    ...
    winners = (
        "\n".join(
            f"- {m.get('content','')[:80]} → {m.get('likeCount', m.get('playCount', 0))} plays"
            for m in ana[:3]
        )
        or "none yet"
    )
    ...
    prompt = f"""You are the viral editor of an Indian news Instagram page.
...
def extract_schema(state):
    ...
    real_quotes = (
        "\n".join(f'- "{q}"' for q in scraped.get("quotes", []))
        or "No direct quotes found."
    )

    real_date = scraped.get("date", "today")

    lang_hint = (
        "narration lines MUST be in simple spoken Hindi, Devanagari script. "
...
Also write caption + 8 hashtags."""
    ...
    if len(schema.get("scenes", [])) < 3:
        ...
        schema["scenes"] = [
            ...
            Scene(
                type="breaking",
                breaking_headline=_cut(t, 60).upper(),
                breaking_sub=a["source"],
                breaking_image_query="news",
                narration=t,
            ).model_dump(),
        ]
...
def _enforce_truth(scenes, state):
    ...
    # M008: force at least one clip scene
...
def proofread_schema(state):
    ...
    for scene in schema.get("scenes", []):
        if (
            scene.get("type") == "breaking"
            and scene.get("breaking_sub", "").upper().startswith(a["source"].upper())
        ):
    ...
    # Remove duplicate quote scenes at dict level too
    seen_q = set()
    clean = []

    for scene in schema.get("scenes", []):
        if scene.get("type") == "quote":
...
def render_scenes(state):
    ...
    # Attach real images + article links
    for sc in scenes:
        if sc.type in ("article", "breaking") and not sc.image_url:
            ...
            hit = next(
                (
                    ...
                    if sum(w in a["title"].lower() for w in target.split()[:5]) >= 2
                ),
                None,
            )
...
def select_format(state):
    ist_now = datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(
        hours=5,
        minutes=30,
    )
...
def extract_roundup(state):
    listing = "\n".join(
        f"[{i}] {a['source']}: {a['title']}"
        for i, a in enumerate(state["articles"][:15])
    )
...
Create a fast-paced "Top 8 Headlines" reel.
...
- Never use graphic, gory, or disturbing imagery descriptions."""
    ...
    items = resp.scenes or [
        RoundupScene(
            headline=_cut(x["title"], 60).upper(),
            narration=x["title"],
            image_query="news",
        )
        for x in state["articles"][:8]
    ]

    scenes = [
        Scene(
            type="breaking",
            breaking_headline="INDIA IN LAST 24 HOURS",
            breaking_sub="TOP 8 HEADLINES",
            breaking_image_query="india news studio",
            narration=resp.intro_narration or "आइए जानते हैं आज की बड़ी खबरें",
        ).model_dump()
    ]
...
def reply_comments(state):
    ...
    try:
        ...
        comments_res = httpx.get(
            f"{ZERNIO}/comments",
            params={"postId": post_ids[0], "limit": 5},
            headers={"Authorization": f"Bearer {settings.zernio_api_key}"},
            timeout=30,
        ).json()
        ...
        for c in comments:
            ...
            reply_resp = llm_create(
                f"Write a 1-sentence friendly reply to this Instagram comment from {c_user}: '{c_text}'. "
                f"Ask a question back. Reply ONLY with the text.",
                CommentReply,
            )
