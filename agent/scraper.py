...
def deep_scrape(url):
    """Scrape real article body, quotes, date, and author."""
    raw = get_html(url)
    if not raw:
...
def embedded_videos(url):
    """Find real embedded videos from the article page."""
    raw = get_html(url)
    if not raw:
        return []
    ...
    urls += [f"https://www.youtube.com/watch?v={m}" for m in yt_ids]
...
def clip_from_urls(urls, path, dur=8):
    """Download a short real embedded clip from article embeds."""
    if not urls:
        return None
...
def page_screenshot(url, path):
    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            try:
                pg.locator("article, main, h1").first.screenshot(
                    path=path,
                    timeout=5000,
                )
            except Exception:
...
def geocode(location):
    """Return lat, lon, and detected country for a pin."""
    if not location:
        return None, None, ""
...
KARAOKE_CSS = (
    ".w{position:relative;display:inline-block;margin-right:.25ch}"
    ".w i{position:absolute;left:-2px;right:-.35ch;top:6%;height:88%;"
    "background:#d40000;opacity:.92;transform:scaleX(0);"
    "transform-origin:left;animation:hl .16s forwards}"
...
def mobile_record(url, name, dur, delays=None, scroll=False):
    """
    Record a real article page like a human phone screen.

    Important M010 fix:
    - We DO NOT use record_video_size.
    - Playwright records native 540x960.
    - editor.py upscales to 1080x1920 with lanczos.
    """
    from playwright.sync_api import sync_playwright
    ...
    css = (
        "*:not(i){animation:none!important;transition:none!important}"
        + HIDE_JUNK_CSS
        + (KARAOKE_CSS if delays is not None else "")
    )
                ...
                pg.evaluate(
                    ...
                    (delays) => {
                        ...
                        h.innerHTML = words.map((w, i) =>
                            `<span class="w"><i style="animation-delay:${(delays[i] || i * 0.35).toFixed(2)}s"></i><b>${esc(w)}</b></span>`
                        ).join(' ');
                    }
                    """,
                    delays,
...
def main_image_url(url):
    """Find the biggest real image in the article."""
    try:
        from playwright.sync_api import sync_playwright
        ...
        with sync_playwright() as pw:
            ...
            src = pg.evaluate(
                ...
                () => {
                    ...
                    document.querySelectorAll('article img, main img, figure img, img')
                        .forEach(im => {
...
def commons_video(query, path):
    ...
    try:
        ...
        if r.status_code != 200 or "json" not in r.headers.get("content-type", ""):
            logger.warning(f"commons video non-JSON ({r.status_code}) → skip")
            return None
        ...
        for p in ((data.get("query") or {}).get("pages") or {}).values():
            ...
            if "video" in (ii.get("mime") or ""):
                if media.download(ii.get("url"), path, 240):
                    return path
